...
class model():
    ...
    def model(self, train_first, train_second):
        ...
        with tf.variable_scope("test_parameters"):
            self.test_para_embedding = tf.Variable(initial_value=tf.truncated_normal(shape=[25000, 400]),
                                                   trainable=True,
        ...
        self.mlp_loss_op = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(labels=mlp_labels, logits=mlp_output))
        classification_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "classification_parameters")
        self.mlp_train_op = tf.train.AdamOptimizer(learning_rate=0.02).minimize(self.mlp_loss_op,
                                                                                var_list=classification_var)
